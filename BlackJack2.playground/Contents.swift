...
func getHitCount(count: Int, deal: Deal, nums: [Int]) -> Int? {
    ...
    if (deal.didPlayerWin()) {
        hitCount += 1
        return hitCount
    } else if (!(deal.isPlayerBust())) {
        let restNums: [Int] = ([Int])(nums[1...(nums.count - 1)])
