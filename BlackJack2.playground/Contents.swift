...
func getMaxWinCount(node: Node, count: Int) -> Int {
    ...
    if (node.childs.count > 0) {
        for childNode in node.childs {
            ...
            print("## subCount : " + String(subCount))
...
//    var rootNode = createDealNode(depth: depth, node: DealNode(deal: Deal(), restNums: nums))
    var rootNode = getNode(rootDeal: Deal(), nums: nums, depth: 1)
    var maxWinCount = getMaxWinCount(node: rootNode, count: 0)
    print("### max winCount: " + String(maxWinCount))
    
//    var winCountList = getWinCountList(node: rootNode, count: 0)
