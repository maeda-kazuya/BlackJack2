class Deal {
    ...
    func didPlayerWin() -> Bool {
        for playerCount in getPlayerCounts() {
            if (playerCount < 22 && (playerCount > getDealerCount() || getDealerCount() > 21)) {
                return true
            } else {
                continue
            }
...
func capNums(nums: [Int]) -> [Int] {
    var cappedNums: [Int] = []
    
    for num in nums {
        (num > 10) ? cappedNums.append(10) : cappedNums.append(num)
    }
    print (cappedNums)
...
func shouldHit(deal: Deal, nums: [Int]) -> Bool {
    deal.playerNums.append(nums[4])
...
func game(count: Int, nums: [Int]) -> Int {
    var winCount = count
    let deal = Deal()
    ...
    if (deal.didPlayerWin()) {
        ...
        if (nums.count > 7) {
            ...
            winCount = game(count: winCount, nums: restNums)
        }
    } else {
        print("Loosing..")
...
//    }
    ...
    winCount = game(count: winCount, nums: nums)
